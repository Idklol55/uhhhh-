...
class PlayState extends MusicBeatState
{
	...
	private var sanshealthbar:FlxSprite;
	...
	//chara attack
	public static var attackTimer:Float = 5;
	public static var attacking:Bool = false;
	...
	override public function create()
	{
			...
			case 'hall':
				{
					...
					var hall:FlxSprite = new FlxSprite().loadGraphic(Paths.image('hall', 'sans'));
					hall.antialiasing = true;
					hall.scale.set(1.5, 1.5);
					add(hall);
					
					blackUP = new FlxSprite().makeGraphic(0, -120, FlxColor.BLACK);
					blackUP.screenCenter(X);
					blackUP.cameras = [camHUD];
					//blackUP.scrollFactor.set();
					//blackUP.alpha = 0;
					add(blackUP);
		
					blackDown = new FlxSprite().makeGraphic(0, 720, FlxColor.BLACK);
					blackDown.screenCenter(X);
					blackDown.cameras = [camHUD];
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
			{
				...
				if(dad.curCharacter == 'sans')
					songPosBar.createFilledBar(FlxColor.GRAY, FlxColor.fromString('#294876'));
		...
		if(dad.curCharacter == 'sans')
		{
			sanshealthbar = new FlxSprite(0, 670).loadGraphic(Paths.image('healthbar/sanshealthbar'));
			if (FlxG.save.data.downscroll)
				sanshealthbar.y = 50;
			sanshealthbar.screenCenter(X);
			sanshealthbar.scrollFactor.set();
			add(sanshealthbar);
		...
		if (loadRep)
			...
			switch (mcontrols.mode)
			{
				...
				_vpad.alpha = 0.75;
				if (SONG.song.toLowerCase() == 'bopeebo') this.add(_vpad);
				case HITBOX:
	...
	function startSong():Void
	{
		...
		if (FlxG.save.data.songPosition)
		{
			...
			songPosBar.scrollFactor.set();
				if(dad.curCharacter == 'sans')
					songPosBar.createFilledBar(FlxColor.GRAY, FlxColor.fromString('#254C73'));
				//else if(dad.curCharacter == 'tuxsteve' || dad.curCharacter == 'tuxsteveuoh' || dad.curCharacter == 'steve-armor' || dad.curCharacter == 'stevehorse' || dad.curCharacter == 'tuxstevewhatever')
				else
			songPosBar.createFilledBar(FlxColor.GRAY, FlxColor.LIME);
			add(songPosBar);
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (FlxG.keys.justPressed.SPACE #if mobileC || _vpad.buttonA.justPressed #end)
			{
				boyfriend.playAnim('hey');
				dad.playAnim("SansDodge",true);
	...
	function endSong():Void
	{
		...
		else
		{
			if (isStoryMode)
			{
				...
				{
					case 'bopeebo':
						var songFormat = StringTools.replace('pico', " ", "-");
						if (attackedSans)
						{
							songFormat = StringTools.replace('Final-Stretch', " ", "-");
							...
							FlxG.sound.music.stop();
							PlayState.SONG = Song.loadFromJson(poop, 'Final-Stretch');
							//playCutscene2('genocide', 27);
						}
						else
						{
							songFormat = StringTools.replace('pico', " ", "-");
							var poop:String = Highscore.formatSong(songFormat, storyDifficulty);
							...
							FlxG.sound.music.stop();
							PlayState.SONG = Song.loadFromJson(poop, 'pico');
	...
	*/
	...
	override function stepHit()
	{
		...
		if (curSong == 'Final-Stretch')
		{
			switch(curStep)
			{
				case 100:
					FlxTween.tween(blackUP, {y: 0}, 1, {ease: FlxEase.circInOut});
					FlxTween.tween(blackDown, {y: 600}, 1, {ease: FlxEase.circInOut});
					FlxTween.tween(strumLineNotes, {y: 120}, 0.5, {ease: FlxEase.circInOut});
				case 200:
					FlxTween.tween(blackUP, {y: -120}, 1, {ease: FlxEase.circInOut});
					FlxTween.tween(blackDown, {y: 720}, 1, {ease: FlxEase.circInOut});
					FlxTween.tween(strumLineNotes, {y: 50}, 1, {ease: FlxEase.circInOut});
				}
		}
